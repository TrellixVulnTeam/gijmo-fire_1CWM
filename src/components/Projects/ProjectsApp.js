...
import AddColumnDropdown from './../common/AddColumnDropdown';
...
export default class Panel extends React.Component {
  constructor(props) {
    ...
    this.getAddColumnDropdown = this.getAddColumnDropdown.bind(this)
  ...
  getProcessedProjects(projects_obj) {
    return Object.keys(projects_obj).map((key) => {
      return {
        id: key,
  ...
  componentDidMount() {
    ...
    this.store_ref.on('value', (snapshot) => {
      ...
      const projects_list = this.getProcessedProjects(projects_obj);
      const view = new CollectionView(projects_list);
  ...
  onChange(s, e) {
    ...
    for (let i = 0; i < items.length; i++) {
      items[i].id = 'project-'+mongoObjectId()
  ...
  isRowEmpty(project = {}) {
    project = {...project}
    delete project['sel_for_deletion']
  ...
  deleteRows(rows = []) {
    const updates = {}
    const ids = rows.map(project => {
      updates[`/${TABLE_KEY}/` + project['id']] = null
    })
    firebase.ref().update(updates);
  }

  deleteSelected() {
    const selected_rows = this.state.view.items.filter(project => project['sel_for_deletion'])
  ...
  addChild(name, type_id) {
    const viewState = this.state.viewState
    const table_state = JSON.parse(viewState)
    const column_state = JSON.parse(table_state.columnLayout)
    
    column_state.columns.push({
      dataType: type_id<4?type_id:1,
    ...
    firebase.ref().child('views').child(TABLE_KEY).child('allViews')
      .child(this.state.currentView).child('state').set(JSON.stringify(table_state));
    this.setState({viewState: JSON.stringify(table_state)}, function() {
      this.retrieveState();
    })
  }
  ...
  getAddColumnDropdown() {
    return (
      <AddColumnDropdown table={TABLE_KEY} addColumn={this.addChild}/>
    )
  }
